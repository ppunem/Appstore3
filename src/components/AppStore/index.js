...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, userInput: ''}
  ...
  activeTabApps = () => {
    ...
    const activeApps = appsList.filter(each => each.category === activeTabId)
    return activeApps
  }
  ...
  filterApps = apps => {
    ...
    const filteredApps = apps.filter(each =>
      each.appName.toLowerCase().includes(userInput.toLowerCase()),
    )
    return filteredApps
  }

  render() {
    const {userInput, activeTabId} = this.state
    const activeApps = this.activeTabApps()
    const filteredApps = this.filterApps(activeApps)
